/**********************************************************************************************
   ...
   This source is based on John Mechalas documentation "Garmin IMG File Format"
found at sourceforge. The missing bits and error were rectified by the source
code of Konstantin Galichsky (kg@geopainting.com), http://www.geopainting.com
...
quint32 CGarminPolygon::decode(qint32 iCenterLon, qint32 iCenterLat,
  ...
  while (sr.get(x, y)) {
    ...
    //         {
    //             qDebug() << "bam";
    //             qDebug() << xy.u << xy.v << pStart << pEnd << (pEnd - pStart)
    //             << (cnt + 1) << line; assert(0);
    //         }
#ifdef DEBUG_SHOW_POLY_PTS
    qDebug() << xy.u << xy.v << Qt::hex << x1 << y1 << GARMIN_DEG(x1)
...
quint32 CGarminPolygon::decode2(qint32 iCenterLon, qint32 iCenterLat,
  ...
  while (sr.get(x, y)) {
    ...
    if (fabs(xy.v) > 2 * M_PI || fabs(xy.u) > 2 * M_PI) {
      //            qDebug() << "bam";
      //            qDebug() << xy.u << xy.v << pStart << pEnd << (pEnd -
      //            pStart) << (cnt + 1) << line;
      // assert(0);
    }
#ifdef DEBUG_SHOW_POLY_PTS
...
void CGarminPolygon::bits_per_coord(quint8 base, quint8 bfirst, quint32& bx,
  ...
  if (y_sign_same) {
    ...
    //         signinfo.ny         = x_sign_same ? bfirst & 0x08 : bfirst &
    //         0x04;
