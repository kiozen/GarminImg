...
/**********************************************************************************************
    ...
    This file contains macros for platform-independant access of data stored in
little-endian format.
    ...
    Garmin stores data in map files and in their devices in little-endian
format.  The data is usually packed, i.e. it is not guaranteed that multi-byte
data is aligned at 16, 32 or 64 bit boundaries.  This is fine as long as you run
QLandkarte (or any derived software) on a little-endian machine which accepts
accessing unaligned data.

    The `configure' script in the top-level folder tries to detect if your
machine is a little endian (like Intel or ARM) or a big endian (like PowerPC or
Sparc).  In the latter case, it defines the macro HAVE_BIGENDIAN.  In another
test, it checks if your machine supports accessing unaligned memory (like Intel
or PowerPC) or if such accesses would fail (as on ARM or Sparc).  If unaligned
accesses are supported, the macro CAN_UNALIGNED will be defined.  Of course, the
file config.h from the top-level folder has to be included.

    2. How to access data

    To work around these problems, this file defines a number of access macros.
On machines which do not need them, they always expand to nothing, but they are
*absolutely* necessary on others.  So *never* access multi-byte elements without
using these macros.

    2.1 Load data from Garmin

    The following rules apply if you want to access data in a source coming from
a Garmin device or file:

    (a) the source is a constant or an aligned 16, 32 or 64-bit value

    Always use the macro
        gar_endian(<type>, <source>)
    where type may be int16_t, int32_t, int64_t, uint16_t, uint32_t, uint64_t,
float or double. The returned value will explicitly be cast'ed to <type>.
...
float or double. The returned value will explicitly be cast'ed to <type>.
    ...
    where type may be int16_t, int32_t, int64_t, uint16_t, uint32_t, uint64_t,
float or double or the special Garmin types int24_t or uint24_t.  The returned
value will be of type <type> except for the Garmin types int24_t or uint24_t
which will be int32_t or uint32_t, respectively, but have the uppermost 8 bits
always set to 0.
    ...
    where type may be int16_t, int32_t, int64_t, uint16_t, uint32_t, uint64_t,
float or double. if the variable is aligned, use "destination = gar_endian(type,
source)" which is faster.
    ...
    where type may be int16_t, int32_t, int64_t, uint16_t, uint32_t, uint64_t,
float or double or the special Garmin types int24_t or uint24_t.  For a standard
type and an aligned pointer destination, use "*(type *)(ptr) = gar_endian(type,
source)" which is faster.
...
// --------------------------------------------------------------------------------------------
// macros to fix the endianess of the constant or properly aligned argument x of
// type t
...
static inline void __gar_ptr_store_uint24_t(uint8_t *p, uint32_t src) {
  __gar_ptr_store_uint16_t(p, src & 0xffffu);
  p[2] = src >> 16;
}
...
// store argument src of type t in the variable dst of type t - call pointer
// store macro
...
static inline uint32_t __gar_ptr_load_uint24_t(const uint8_t *p) {
#ifdef __powerpc__
...
static inline uint32_t __gar_ptr_load_uint32_t(const uint8_t *p) {
#ifdef __powerpc__
  register uint32_t temp;
...
static inline void __gar_ptr_store_uint32_t(uint8_t *p, uint32_t src) {
  p[0] = src & 0xffu;
  p[1] = (src >> 8) & 0xffu;
  p[2] = (src >> 16) & 0xffu;
  p[3] = (src >> 24) & 0xffu;
