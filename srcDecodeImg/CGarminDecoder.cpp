...
void CGarminDecoder::readMapInfo(QDataStream& stream)
{
    ...
    print("Map Info: %i %i %i %s %s %s %08X %i \n", idProd, idFamily, mapNumber, seriesName.toLocal8Bit().data(), mapDesc.toLocal8Bit().data(), areaName.toLocal8Bit().data(), mapId, mapId);
}
...
void CGarminDecoder::readStringTable(QFile &file, subfile_t &subfile)
{
    ...
    default:
        qWarning() << "Unknown label coding" << hex << subfile.hdrLBL.coding;
    }
        ...
        for(const subdiv_t& subdiv : subfile.subdivs)
        {
            ...
            while(pData < pEnd)
            {
                if(*pData == 0xE0)
                {
                    ...
                    qDebug("--------   tile  --------");
                    qDebug() << tileEntry.index << tileEntry.lon1 << tileEntry.lat1 << tileEntry.lon2 << tileEntry.lat2 << tileEntry.size;
                }
                else
                {
                    ...
                    qDebug("\n-------- polygon --------");
                    qDebug() << "lbl_in_NET" << polygonEntry.lbl_in_NET << "hasV2Label" << polygonEntry.hasV2Label << "lbl_info" << polygonEntry.lbl_info << polygonEntry.labels;
...
void CGarminDecoder::dumpSubfiles()
{
    ...
    for(const subfile_t& subfile : subfiles)
    {
        ...
        {
            CSubfileTask * task = new CSubfileTask([this, &subfile](){
                processSubfile(subfile);
            });
...
void CGarminDecoder::processSubfile(const subfile_t &subfile)
{
    ...
    print(">>> %08X %08X %08X\n", subfile.parts["GMP"].offset, subfile.hdrLBL.lbl28_offset, subfile.parts["GMP"].offset + subfile.hdrLBL.lbl28_offset);
...
//            print("%08X %08X %08X ", subfile.parts["GMP"].offset, subfile.hdrLBL.lbl29_offset, jpgOffset);
//            print("Tile#: %i Offset: %08X %i\n", tileEntry.index, tile.offset, tile.offset);
