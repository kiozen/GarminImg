...
QStringList CFileGmp::copyright1 = {
    "This map has been crated by " WHAT_STR,
...
CFileGmp::CFileGmp(const QString& name) : name(name), tre(name) {
...
void CFileGmp::addCopyright(const QString& str) { tre.addCopyright(lbl.addLabel(str)); }
...
void CFileGmp::addTile(const QString& path, qreal northbound, qreal eastbound, qreal southbound, qreal westbound,
...
void CFileGmp::write(const QDir& path) {
  ...
  for (CMapLevel& maplevel : tre.maplevels()) {
    for (CSubdiv& subdiv : maplevel.subdivs()) {
      if (tre7RecSize == 12) {
        tre.addTre7Record(rgn.addPolygons2(subdiv.polygon2()), rgn.addPolylines2(subdiv.polyline2()),
      ...
      if (tre7RecSize > 0) {
        tre.addTre7Record(rgn.addPolygons2(subdiv.polygon2()));
      }
    }
  }
  tre.finalizeSubdivs();
...
void CFileGmp::remove(const QDir& path) { QFile::remove(path.absoluteFilePath(name + ".gmp")); }
